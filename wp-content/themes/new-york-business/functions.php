...
function new_york_business_general_admin_notice(){
	...
	if($pagenow == 'index.php' || $pagenow == 'themes.php'){
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" >
			 	...
			 	<a href=%9$s target="_blank"  style="text-decoration: none; margin-left:10px;" class="button button-primary">%10$s</a><p></p>
				<a href="#" style="text-decoration: none; margin-left:10px;" class="button button-secondary">%11$s</a>
			 	<a href="?new_york_business_notice_dismissed" target="_self"  style="text-decoration: none; margin-left:10px;" class="button button-secondary">%12$s</a>
			 	</p></div>',
				...
				esc_html__('Use THEME: Product Categories Grid | Slider widget to build your WooCommerce store','new-york-business'),
...
if ( isset( $_GET['new_york_business_notice_dismissed'] ) ){
	update_option('new_york_business_notice', 3);
}

$new_york_business_notice = get_option('new_york_business_notice', 0);
if($new_york_business_notice != 3){
